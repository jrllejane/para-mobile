...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<FloatingActionButton>(R.id.test_route_button).setOnClickListener {
            testSimpleRoute()
        }
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_api_stats -> {
                // Show API usage stats in the navigation item selection
                showApiUsageStats()
                return true
            }
        }
    ...
    private fun showApiUsageStats() {
        searchService.checkApiStatus { stats ->
            // Show the API status in a dialog
            AlertDialog.Builder(this)
                .setTitle("API Usage Statistics")
                .setMessage(stats)  // stats now holds the actual message from the callback
